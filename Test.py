...
data_dir = "../../Data/ml-1m"
results_dir = "../../Data/ml-1m/results"
...
def GetItemsForTest(user, items, item_bias, n_samples):
    ...
    step = int(items.shape[0] / n_samples)
    result = []
...
def TestAllData(method, mode):
    ...
    popular_items = np.genfromtxt(data_dir + "/tes_ratings.txt").astype(int)

    non_popular_items = np.array(
        [i for i in range(item_vecs.shape[0]) if i not in popular_items]
    )

    non_informative_items = np.where(abs(item_bias) > 0.3)[0]

    mean_dist = [0.0 for i in range(n_points + 2)]
    NDCG_train = [0.0 for i in range(n_points + 2)]
    correct_pairs = [0.0 for i in range(n_points + 2)]
    precision10 = [0.0 for i in range(n_points + 2)]
    ...
    for i in range(n_users_in_test_):
        ...
        questions_item = method.RecieveQuestions(
            item_vecs[popular_items[u_r]],
            user_vecs[i],
            user_estim_first.copy(),
            n_points,
            item_bias[popular_items[u_r]],
            ratings[i][popular_items[u_r]],
        )
        questions_item = np.array(questions_item).astype(int)
        # print(f"Question Items: {questions_item}")
        # print(f"ur Arra: {u_r}")
        ...
        # test
        if mode == 0:
            non_zerro_ratings = GetItemsForTest(
                user_vecs[i],
                item_vecs[non_popular_items],
                item_bias[non_popular_items],
                50,
            )
        ...
        for j in range(n_points + 2):
            ...
            dist_b, ndcg_b, correct_pairs_, precision10_ = Test(
                questions_item,
                (j),
...
def RunTest(method):
    ...
    if method == "IG":
        main(
            ClusteringPairwise(
                data_dir + "/users_train.txt",
                data_dir + "/centroids_file",
                data_dir + "/clusters",
                80,
                n_points,
                0,
