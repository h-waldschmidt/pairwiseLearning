...
def GetSimilarUsers(user, user_bias, users_estimation, user_bias_estimation):
    ...
    distance_to_users = [
        [
            np.dot(user - users_estimation[i], (user - users_estimation[i]).T)
            + (user_bias - user_bias_estimation[i]) ** 2,
            i,
        ]
...
def GetReward(
    distance_to_users, users, users_bias, item, item_bias, global_bias, sigma
):
    n_users = 0
    result = 0
    i = 0
    while i < users.shape[0] and distance_to_users[i][0] < 0.01:
        ...
        result += int(
            recieveAnswer(
                users[distance_to_users[i][1]],
                users_bias[distance_to_users[i][1]],
                item,
                item_bias,
                global_bias,
                sigma,
...
def GetData(datadir):
    ...
    user_vecs_train = np.genfromtxt(datadir + "/users.txt")
    ...
    return item_vecs, item_bias, user_vecs_test, user_vecs_train


# , , user_bias_test, global_bias, , user_bias_train
...
def make_input(item_vec, item_bias, user_vec, user_bias):
    return appendAllArrays(
        [item_vec, item_bias, user_vec, user_bias, item_vec * user_vec]
    )
...
def GetBestItem(item_vecs, item_bias, user, user_bias, W, used_items):
    ...
    for item1 in range(item_vecs.shape[0]):
        c = np.dot(W, make_input(item_vecs[item1], item_bias[item1], user, user_bias))
...
def PrintItemPopularity(user_vecs, user_bias, item_vecs, item_bias, global_bias, sigma):
    sort_items = SortIItemByPopularity(
        user_vecs, user_bias, item_vecs, item_bias, global_bias, sigma
    )
        ...
        for i in sort_items:
            try:
                if float(items_names[items_test[i[2]] + 1][1]) > 0:
                    ...
                    ratings.append(float(items_names[items_test[i[2]] + 1][1]) / 10.0)
                    ratings1.append(i[0])
            except:
                continue
...
def DeleteMostPopularItems(n_delete_items):
    ...
    with open("data/items.txt") as items, open("data/items_bias.txt") as i_b, open(
        "data/items1.txt", "w"
    ) as n_items, open("data/items_bias1.txt", "w") as n_ib:
        n_line = 0
        for line in items:
...
def GetInverseMatrix(previousMatrix, new_vector):
    ...
    return previousMatrix - (np.dot(previousMatrix, v) / (1.0 + np.trace(v)))
